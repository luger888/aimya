 ...
 * @package    Aimya_Filter
 */
class Aimya_Filter_File_Resize implements Zend_Filter_Interface
{
    ...
    protected $_adapter = 'Aimya_Filter_File_Resize_Adapter_Gd';
     ...
     * @return Aimya_Filter_File_Resize An instance of this filter
     */
    public function __construct($options = array())
    {
        ...
        if (isset($options['adapter'])) {
            if ($options['adapter'] instanceof Aimya_Filter_File_Resize_Adapter_Abstract) {
                $this->_adapter = $options['adapter'];
            } else {
                $name = $options['adapter'];
                if (substr($name, 0, 33) != 'Aimya_Filter_File_Resize_Adapter_') {
                    $name = 'Aimya_Filter_File_Resize_Adapter_' . ucfirst(strtolower($name));
                }
                $this->_adapter = $name;
            }
        }
    ...
    {
        if ($this->_adapter instanceof Aimya_Filter_File_Resize_Adapter_Abstract) {
            return;
        } else {
